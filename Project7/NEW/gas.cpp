...
void doBasicTests()
{
  ...
  cerr << ousDarth.nFlatulansAt(1, 2) << " " << MAXFLATULANS << endl;
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    for (int i = 0; i < m_nFlatulans; i++){
        ...
        if (flatR == playerR + 1 && flatC == playerC && check){
            delete m_flatulans[i];
            ...
            i--;
        }else if (flatR == playerR - 1 && flatC == playerC && check){
            delete m_flatulans[i];
            ...
            i--;
        }else if (flatR == playerR && flatC == playerC + 1  && check){
            delete m_flatulans[i];
            for (int j = i; j < m_nFlatulans-1; j++){
                m_flatulans[j] = m_flatulans[j+1];
            }
            ...
            i--;
        }else if (flatR == playerR - 1 && flatC == playerC - 1  && check){
            delete m_flatulans[i];
            for (int j = i; j < m_nFlatulans-1; j++){
                m_flatulans[j] = m_flatulans[j+1];
            }
...
void City::moveFlatulans()
{
    for (int k = 0; k < m_nFlatulans; k++)
    {
        ...
        int flatR = m_flatulans[k]->row();
        int flatC = m_flatulans[k]->col();

        int playerR = m_player->row();
        int playerC = m_player->col();

        if (flatR == playerR + 1 && flatC == playerC){
            m_player->getGassed();
        }else if (flatR == playerR - 1 && flatC == playerC){
            m_player->getGassed();
        }else if (flatR == playerR && flatC == playerC + 1 ){
            m_player->getGassed();
        }else if (flatR == playerR && flatC == playerC - 1 ){
            m_player->getGassed();
        }
